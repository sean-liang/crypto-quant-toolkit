...
class OKExEquityCurve:
    ...
    def calculate(self, df, **kwargs):
        ...
        df['contract_num'] = np.where(open_pos_cond, self._contract_num(df[CANDLE_OPEN_COLUMN]), np.NaN)
        # 根据滑点计算实际开仓价格
        ...
        # 平仓手续费
        df['close_pos_fee'] = np.where(close_pos_cond, df['close_pos_price'] * self._face_value * df['contract_num'] * self._commission, np.NaN)
        ...
        df['profit'] = np.where(close_pos_cond, self._face_value * df['contract_num'] * (df['close_pos_price'] - df['open_pos_price']) * df[POSITION_COLUMN],
                                df['profit'])
        ...
        # 平仓时扣除手续费
        df['net_value'] = np.where(close_pos_cond, df['net_value'] - df['close_pos_fee'], df['net_value'])
        # df.loc[close_pos_cond, 'net_value'] -= df['close_pos_fee']

        # 计算爆仓
        df['price_min'] = np.where(df[POSITION_COLUMN] == 1, df[CANDLE_LOW_COLUMN], np.NaN)
        df['price_min'] = np.where(df[POSITION_COLUMN] == -1, df[CANDLE_HIGH_COLUMN], df['price_min'])
        ...
        df['blow_up'] = np.where(df['margin_ratio'] <= (self._min_margin_ratio + self._commission), 1, np.NaN)
        # df.loc[df['margin_ratio'] <= (self._min_margin_ratio + self._commission), 'blow_up'] = 1  # 计算是否爆仓

        # 当下一根K线价格突变，在平仓的时候爆仓，要做相应处理。此处处理有省略，不精确。
        df['blow_up'] = np.where(close_pos_cond & (df['net_value'] < 0), 1, df['blow_up'])
